...
import os
import json
from typing import List, Type, Dict, Any, Union, Tuple, Optional
...
class Simulator:
    def __init__(
        self,
        data_dir: str = None,
        block_set_dir: str = None,
        device: str = "auto",
        cache: bool = False,
        semantic_id_artifact_dirs: Optional[Dict[str, str]] = None,
        default_sid_source: str = "amazon",
    ):
        """
        ...
        Args:
            ...
            semantic_id_artifact_dirs: Optional mapping of source name to artifact directory
                produced by scripts/build_semantic_ids.py, e.g.
                {"amazon": "artifacts/amazon", "yelp": "artifacts/yelp"}.
            default_sid_source: Default source used by SemanticIDTool when item metadata
                does not specify one.
        """
        logger.info("Start initializing Simulator")
        self.data_dir = data_dir
        self.block_set_dir = block_set_dir
        self.default_sid_source = default_sid_source
        if data_dir is None:
        ...
        self.semantic_id_tool = None
        if semantic_id_artifact_dirs:
            self.set_semantic_id_tool(
                artifact_dirs=semantic_id_artifact_dirs,
                default_source=default_sid_source,
            )
        
        self.tasks = []  # List to store tasks
        self.groundtruth_data = []  # List to store groundtruth data
        self.task_contexts = []  # Optional per-task history visibility metadata
    ...
    def set_semantic_id_tool(
        self,
        artifact_dirs: Optional[Dict[str, str]] = None,
        semantic_id_tool: Optional[SemanticIDTool] = None,
        default_source: Optional[str] = None,
    ):
        """
        Configure semantic ID tools for GR-aware agents.

        Provide either ``artifact_dirs`` or a pre-built ``semantic_id_tool`` instance.
        """
        if semantic_id_tool is not None:
            self.semantic_id_tool = semantic_id_tool
        elif artifact_dirs is not None:
            self.semantic_id_tool = SemanticIDTool(
                artifact_dirs=artifact_dirs,
                interaction_tool=self.interaction_tool,
                default_source=default_source or self.default_sid_source,
            )
        else:
            raise ValueError("Provide artifact_dirs or semantic_id_tool.")

        if default_source is not None:
            self.default_sid_source = default_source
            self.semantic_id_tool.set_default_source(default_source)

        if self.interaction_tool is not None:
            self.semantic_id_tool.set_interaction_tool(self.interaction_tool)
        logger.info(
            "SemanticIDTool configured for sources: %s",
            self.semantic_id_tool.available_sources(),
        )

    def _attach_tools_to_agent(self, agent):
        agent.set_interaction_tool(self.interaction_tool)
        if self.semantic_id_tool is not None:
            agent.set_semantic_id_tool(self.semantic_id_tool)
    ...
    def set_task_and_groundtruth(self, task_dir: str, groundtruth_dir: str):
        ...
        for task_file in task_files:
            ...
            self.task_contexts.append({})

        logger.info(f"Loaded {len(self.tasks)} task-groundtruth pairs")

    def filter_tasks(self, indices: List[int]):
        """Keep only the selected task indices (0-based, aligned with task_N.json)."""
        if not indices:
            raise ValueError("filter_tasks requires at least one task index.")

        total = len(self.tasks)
        for index in indices:
            if index < 0 or index >= total:
                raise IndexError(f"Task index {index} out of range for {total} loaded tasks.")

        self.tasks = [self.tasks[index] for index in indices]
        self.groundtruth_data = [self.groundtruth_data[index] for index in indices]
        self.task_contexts = [self.task_contexts[index] for index in indices]
        logger.info("Filtered to %s tasks", len(self.tasks))

    def set_task_contexts(self, contexts_by_index: Dict[int, Dict[str, Any]]):
        """Attach per-task history metadata keyed by original task index."""
        updated = []
        for task_index in range(len(self.tasks)):
            updated.append(contexts_by_index.get(task_index, {}))
        self.task_contexts = updated

    def _apply_task_context(self, task_index: int):
        if task_index >= len(self.task_contexts):
            return
        context = self.task_contexts[task_index] or {}
        if hasattr(self.interaction_tool, "set_task_context"):
            self.interaction_tool.set_task_context(
                task_time=context.get("task_time"),
                history_window_days=context.get("history_window_days"),
                history_window_count=context.get("history_window_count"),
            )
    ...
    def run_simulation(self, number_of_tasks: int = None, enable_threading: bool = False, max_workers: int = None, time_limitation: float = None) -> List[Any]:
        ...
        if not enable_threading:
            ...
            for index, task in enumerate(task_to_run):
                ...
                self._attach_tools_to_agent(agent)
                self._apply_task_context(index)
