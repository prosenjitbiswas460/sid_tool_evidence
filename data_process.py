...
def merge_review_data(yelp_reviews, amazon_reviews, goodreads_reviews, output_file=None):
    ...
    if 'review_id' not in goodreads_reviews.columns:
        goodreads_reviews['review_id'] = [
            str(uuid.uuid4()) for _ in range(len(goodreads_reviews))
        ]

    timestamp_source = None
    for candidate in ('date_updated', 'date_added', 'read_at', 'started_at'):
        if candidate in goodreads_reviews.columns:
            timestamp_source = candidate
            break
    if timestamp_source is not None:
        parsed_dates = pd.to_datetime(
            goodreads_reviews[timestamp_source],
            errors='coerce',
            utc=True,
        )
        goodreads_reviews['timestamp'] = (
            parsed_dates.view('int64') // 10**9
        ).where(parsed_dates.notna(), other=None)
        logging.info(
            "Goodreads timestamps derived from column: %s",
            timestamp_source,
        )
    else:
        logging.warning(
            "Goodreads reviews have no date_updated/date_added/read_at column; "
            "time-based conditions will not work until raw dates are present."
        )
